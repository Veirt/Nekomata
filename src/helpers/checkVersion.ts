...
export default (client: Discord.Client, channels: Channel[]) => {
  fs.readFile(path.join(__dirname, "../../PatchInfo.json"), (err, data) => {
    ...
    patchInfos.forEach(async (patchInfo) => {
      ...
      const prev = await getConnection().getRepository(Version).findOne({ server: patchInfo.server })
      ...
      if (prev) {
        await getConnection().getRepository(Version).update({ server: patchInfo.server }, { version: nextVersion })
      } else {
        await getConnection().getRepository(Version).save({ server: patchInfo.server, version: nextVersion })
      }
